...
class Reader:
    """Read INTERLIS 2.4 XTF into model-specific dynamic dataclasses."""
    ...
    def _canonicalized_input(self, input_xtf: str | IO[AnyStr]) -> bytes:
        ...
        Some payloads use topic-qualified local names (for example
        ``Topic.Class``). xsdata field metadata is based on the class element
        name, so we strip the topic prefix during parsing.
        """
